    ...
    tweets
      ...
      .forEach((tweet) => {
        ...
        const $username = $("<p>")
          ...
          .text(`@${tweet.user}`)
          .on("click", () => {
            const filteredTweets = streams.home.filter(
              (t) => t.user === tweet.user
            );
            showFilteredTweets(filteredTweets, `@${tweet.user}`);
          });

        const $message = $("<p>").text(tweet.message).css("color", "#b3b3b3");

        const $timestamp = $("<p>")
          .text(
            `${dayjs(tweet.created_at).format("MMMM D, YYYY h:mm A")} (${dayjs(
              tweet.created_at
            ).fromNow()})`
          )
          .css("color", "#888");
